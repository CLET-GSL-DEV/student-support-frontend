...
| Aggregate usage analytics (§2.3, §2.6)                               | `/analytics` + dashboard | `features/analytics/*`, `data/analytics/*`, `api/analytics.ts`, `types/analytics.ts`                                      | Analytics source unnamed in SRS (TODO)     | Built, placeholder aggregates   |
...
| Portal landing (entry points, aggregate summary)                     | `/dashboard`             | `features/dashboard/*`, `constants/adminNav.ts`                                                                           | Reads the analytics seam                   | Built                           |
...
- **Least-privilege**: `ProtectedRoute` gates the app by role; every area page additionally wraps in
  `CapabilityGate` over the `ROLE_CAPABILITIES` model (`constants/admin.ts`).
...
| 4   | Dashboard/analytics metric set (all aggregates are placeholders)                                                                    | `types/analytics.ts`, `data/analytics/mock.ts`, `features/dashboard/components/MetricsSummary.tsx`, `features/analytics/*` |
